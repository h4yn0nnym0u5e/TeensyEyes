...
typedef struct {
  ...
  bool frameBufferInPSRAM; // allocate framebuffer in PSRAM
} ST7789_csFn_Config;

class ST7789_csFn_Display : public Display<ST7789_csFn_Display> {
private:
  ST7789_t3* display;
  bool asyncUpdates;
  int displayNum;
  uint16_t* PSRAMframeBuffer;
...
public:
  ...
  ST7789_csFn_Display(const ST7789_csFn_Config &config);

  virtual ~ST7789_csFn_Display();
